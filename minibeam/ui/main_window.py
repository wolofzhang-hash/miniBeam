...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        sec = Section(name="Rect100x10", type="RectSolid", A=sp.A, Iy=sp.Iy, Iz=sp.Iz, J=sp.J, c_z=sp.c_z, c_t=sp.c_t, Zp=sp.Zp)
    ...
    def _context_add_constraint(self, dof: str):
        ...
        map_dof_to_ui = {"DX": "DX (UX)", "DY": "DY (UY)", "RZ": "RZ", "RX": "RX"}
    ...
    def _context_add_nodal_load(self, load_type: str):
        ...
        if load_type == "FY":
            ...
            self.cmb_load_type.setCurrentText("Nodal MX")
    ...
    def apply_constraint_to_selected_points(self):
        ...
        dof = {"DY (UY)": "DY", "RZ": "RZ", "DX (UX)": "DX", "RX": "RX"}[typ]
    ...
    def apply_load_to_selection(self):
        ...
        if typ.startswith("Nodal"):
            ...
            direction = "FY" if "FY" in typ else ("MX" if "MX" in typ else "MZ")
            for uid in pids:
    ...
    def export_results_csv(self):
        ...
        if has_diag:
            ...
            V = np.asarray(getattr(out, "V", np.zeros_like(xg)), dtype=float)[mask]
            M = np.asarray(getattr(out, "M", np.zeros_like(xg)), dtype=float)[mask]
            T = np.asarray(getattr(out, "T", np.zeros_like(xg)), dtype=float)[mask]
            sigma = np.asarray(getattr(out, "sigma", np.zeros_like(xg)), dtype=float)[mask]
            tau_t = np.asarray(getattr(out, "tau_t", np.zeros_like(xg)), dtype=float)[mask]
            sigma_eq = np.asarray(getattr(out, "sigma_eq", np.zeros_like(xg)), dtype=float)[mask]
            margin = np.asarray(getattr(out, "margin", np.zeros_like(xg)), dtype=float)[mask]
            margin_plastic = np.asarray(getattr(out, "margin_plastic", np.zeros_like(xg)), dtype=float)[mask]
        else:
            x = dy = rz = rx = V = M = T = sigma = tau_t = sigma_eq = margin = margin_plastic = np.array([], dtype=float)
        import csv
        try:
            with open(fn, "w", newline="", encoding="utf-8-sig") as f:
                w = csv.writer(f)
                w.writerow(["TYPE", "name", "combo", "x_mm", "dy_mm", "rz_rad", "rx_rad", "Rxn_FX_N", "Rxn_FY_N", "Rxn_MZ_Nmm", "Rxn_MX_Nmm", "V_N", "M_Nmm", "T_Nmm", "sigma_bend_Nmm2", "tau_t_Nmm2", "sigma_eq_Nmm2", "MS", "MS_plastic"])
                node_rows = []
                for i, p in enumerate(pts_sorted, start=1):
                    r = out.reactions.get(p.name, {}) if getattr(out, "reactions", None) else {}
                    node_rows.append([
                        "NODE", p.name, "", f"{x_nodes[i-1]:.6f}", f"{dy_nodes[i-1]:.9g}", "", "",
                        f"{float(r.get('FX', 0.0)):.9g}", f"{float(r.get('FY', 0.0)):.9g}", f"{float(r.get('MZ', 0.0)):.9g}", f"{float(r.get('MX', 0.0)):.9g}",
                        "", "", "", "", "", "", "", ""
                    ])
                diag_rows = []
                for i in range(len(x)):
                    diag_rows.append([
                        "DIAG", "", out.combo, f"{x[i]:.6f}", f"{dy[i]:.9g}", f"{rz[i]:.9g}", f"{rx[i]:.9g}",
                        "", "", "", "",
                        f"{V[i]:.9g}", f"{M[i]:.9g}", f"{T[i]:.9g}", f"{sigma[i]:.9g}", f"{tau_t[i]:.9g}", f"{sigma_eq[i]:.9g}", f"{margin[i]:.9g}", f"{margin_plastic[i]:.9g}"
                    ])
                if not diag_rows:
                    for r in node_rows:
                        w.writerow(r)
                    ...
                    for row in node_rows:
                        ...
                        if idx >= 0 and abs(float(x_vals[idx]) - x_node) <= 1e-6:
                            merged = list(diag_rows[idx])
                            merged[0] = row[0]; merged[1] = row[1]; merged[2] = row[2]; merged[3] = row[3]; merged[4] = row[4]
                            merged[7] = row[7]; merged[8] = row[8]; merged[9] = row[9]; merged[10] = row[10]
                            diag_rows[idx] = merged
                        else:
                            diag_rows.append(row)
                    for r in sorted(diag_rows, key=lambda rr: (float(rr[3]), 0 if rr[0] == "NODE" else 1)):
                        w.writerow(r)
        except Exception as e:
    ...
    def edit_constraints_selected(self):
        """Open a single dialog to edit DX/DY/RZ/RX constraints for selected point(s).
    ...
    def edit_nodal_loads_selected(self):
        ...
        cb_mx, sb_mx = mk_row("MX / Torque (N·mm)", mx_en, mx_val)
        buttons = QDialogButtonBox(QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
        buttons.accepted.connect(dlg.accept)
        buttons.rejected.connect(dlg.reject)
        lay.addWidget(buttons)
