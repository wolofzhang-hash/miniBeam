...
class TestMSCalculation(unittest.TestCase):
    ...
    def test_non_circular_section_uses_corner_stress_superposition(self):
        result = compute_ms_from_internal_forces(
            N=np.array([0.0]),
        ...
        self.assertAlmostEqual(float(result["sigma_elastic"][0]), 20.0, places=6)
        self.assertAlmostEqual(float(result["sigma"][0]), 20.0, places=6)

    def test_circular_section_uses_vector_synthesis(self):
        result = compute_ms_from_internal_forces(
            N=np.array([0.0]),
            Mz=np.array([1_000_000.0]),
            My=np.array([-1_000_000.0]),
        ...
        self.assertAlmostEqual(float(result["sigma_elastic"][0]), np.sqrt(200.0), places=6)
