...
class TestMSCalculation(unittest.TestCase):
    def test_typical_case_matches_document_example(self):
        ...
        self.assertAlmostEqual(float(result["sigma"][0]), 94.01154, places=4)
        self.assertAlmostEqual(float(result["tau_t"][0]), 75.0, places=6)
        self.assertAlmostEqual(float(result["margin_elastic"][0]), 0.230997, places=4)
        self.assertAlmostEqual(float(result["margin"][0]), 0.434358, places=4)
